  ...
  main.variable(observer("week_id_array")).define("week_id_array", ["week_array"], function(week_array)
{
  ...
  main.variable(observer("if_today_is_in_week")).define("if_today_is_in_week", ["week_id_array","week"], function(week_id_array,week)
{
  let weeks = []
  const today = new Date('2019-05-25')
  let counter = -1
  for( const id of week_id_array) {
    const end = new Date(week[id].end) 
    weeks.push([(today - end) < 0, end])
    counter++
  }
  return weeks
}
);
  main.variable(observer("current_week_counter")).define("current_week_counter", ["week_id_array","week","today"], function(week_id_array,week,today)
{
  let weeks = []
  let counter = -1
  for( const id of week_id_array) {
